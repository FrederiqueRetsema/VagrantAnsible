...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "linuxSlave" do |linuxSlave|
    linuxSlave.vm.box = "generic/centos7"

    linuxSlave.vm.synced_folder ".", "/vagrant"
    linuxSlave.vm.hostname = "linuxSlave"
    linuxSlave.vm.provider "virtualbox" do |vb|
      # Name of this VM
      vb.name = "Ansible slave node (CentOS7)"
      vb.customize ["modifyvm", :id, "--memory", "2048"]
      vb.customize ["modifyvm", :id, "--cpus", "2"]   
    end
    linuxSlave.vm.network "private_network", type: "dhcp"
  end
  config.vm.define "windowsSlave" do |windowsSlave|
    # Name of this VM
    windowsSlave.vm.box = "jacqinthebox/windowsserver2016"

    windowsSlave.vm.hostname = "dc"
